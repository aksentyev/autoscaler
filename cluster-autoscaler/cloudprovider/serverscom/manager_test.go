...
func TestManager_GetNodeGroups(t *testing.T) {
	...
	assert.Equal(t, "kube2-kafka-mow-3.int.avs.io", ng.nodes[0].Id)
	assert.Equal(t, "kube2-kafka-mow-2.int.avs.io", ng.nodes[1].Id)
	assert.Equal(t, "kube2-kafka-mow-1.int.avs.io", ng.nodes[2].Id)
	assert.EqualValues(t, cloudprovider.InstanceRunning, ng.nodes[0].Status.State)
	assert.NotNil(t, ng.nodes[0].Status.ErrorInfo)
	assert.EqualValues(t, cloudprovider.InstanceCreating, ng.nodes[1].Status.State)
	assert.EqualValues(t, cloudprovider.InstanceRunning, ng.nodes[2].Status.State)
}
